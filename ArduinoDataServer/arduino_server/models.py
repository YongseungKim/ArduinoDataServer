...
from arduino_server import signal_receivers
...
class Meter(models.Model):
    ...
    meter_type = models.ForeignKey(MeterType, on_delete=models.CASCADE, help_text=_(
        u"Select a type of meter. "
        u"For instance you could call all your electricity meters 'Electricity'. "
        u"This looks good in the front-end!"))
    default_interval = models.ForeignKey('IntervalType', related_name='default_meter', on_delete=models.CASCADE,
                                         null=True, blank=True,
                                         help_text=_(
                                             u"This is the summary that will most likely be displayed in overviews."))

    is_counter = models.BooleanField(default=False,
                                     verbose_name=_(u"is a counter"),
                                     help_text=_(
                                         u"Indicates that the meter is an incremental counter, "
                                         u"ie. that every time a new number is received, "
                                         u"the difference from the previous number should be logged. "
                                         u"If a number is smaller than the previous, "
                                         u"the data server assumes that counting has been reset, "
                                         u"for instance because the Arduino has rebooted. "
                                         u"This does not affect the counting a summarising procedure."))

    description = models.TextField(blank=True)

    unit_name = models.CharField(max_length=255,
                                 help_text=_(u"Name the unit that's being summarised, for instance 'KWh'."), blank=True,
                                 null=True)
    unit_fraction = models.FloatField(default=1, help_text=_(
        u"If you are doing an annual summary, "
        u"you might want to use a different unit than theone from the data entries. "
        u"If for instance, you are receiving KWh data, you could enter 0.001 and input 'MWh' in the 'unit name' field. "
        u"Note that this fraction will be applied to ALL meters in the summary, "
        u"so they should be outputting the same type of unit."))
...
class MeterData(models.Model):
    ...
    diff = models.FloatField(help_text=_(
        u"This field contains the difference (delta) from the last reading. "
...
class IntervalType(models.Model):
    name = models.IntegerField(choices=INTERVALS, help_text=_(
        u"This is the period or interval in which the summary should automatically summarise every new data entry."))
    verbose_name = models.CharField(max_length=255, help_text=_(
        u"You may give the interval a friendly name, otherwise an auto-generated name is used."),
                                    blank=True, null=True)

    meter_set = models.ManyToManyField(Meter, help_text=_(
        u"Choose multiple meters if you want this to be a summary of multiple meters. "
        u"Please note that this would require the units of these meters to be the same!"),
                                       blank=True)
    unit_name = models.CharField(max_length=255,
                                 help_text=_(u"Name the unit that's being summarised, for instance 'KWh'."))
    unit_fraction = models.FloatField(default=1, help_text=_(
        u"If you are doing an annual summary, you might want to use a different unit than the one from the data entries. "
        u"If for instance, you are receiving KWh data, you could enter 0.001 and input 'MWh' in the 'unit name' field. "
        u"Note that this fraction will be applied to ALL meters in the summary, so they should be outputting the same type of unit."))

    backlog = models.IntegerField(default=0, verbose_name=_(u"Backlog"),
                                  help_text=_(
                                      u"Number of days to keep old entries for. "
                                      u"If you create a new summary and have the necessary data entries, summaries will be created until this point in time."))

    force_recreate = models.BooleanField(default=False,
                                         help_text=_(
                                             u"If checked, this will force a one-time recreation of all summaries based on available data entries "
                                             u"until the point in time that is according to the backlog number of days above. "
                                             u"If you are creating a new interval or have changed the meters included in an interval, this is quite useful! "
                                             u"Correcting data entries does not necessitate recreating a whole interval."))

    def __unicode__(self):
        meters = self.meter_set.all()
        if meters:
            return _(u"%(interval)s summary for %(meters)s") % {
                'interval': unicode(self.get_name_display()).capitalize(),
...
class Interval(models.Model):
    ...
    interval_type = models.ForeignKey(IntervalType, on_delete=models.CASCADE)
